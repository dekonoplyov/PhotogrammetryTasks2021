...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        if (octave == 0) {
            ...
            cv::Size automaticImgSize = cv::Size(0, 0);
            cv::resize(img, img, automaticImgSize, 0.5, 0.5, cv::INTER_NEAREST);
            gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer] = img;
        }

        const double sigmaBase = INITIAL_IMG_SIGMA * pow(2.0, octave) * pow(k, 0);
        ...
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            double sigma = sqrt(sigmaCur*sigmaCur - sigmaBase*sigmaBase);                // sigma2  - сигма которую надо добавить чтобы довести sigma1 до sigma12
    ...
    for (ptrdiff_t octave = 0; octave < NOCTAVES; ++octave) {
        for (size_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            // и понадеяться, что ребята в OpenCV сделали его cache-friendly
...
namespace {
    ...
    Shift parabolaFitting(float x0, float x1, float x2) {
    ...
    PolarCoord calcPolarCoord(const cv::Mat& img, int x, int y) {
        ...
        rassert(orientation >= 0.0 && orientation < 360.0, 13532452345);

        return PolarCoord{magnitude, orientation};
    }
}
            ...
            for (size_t layer = 1; layer + 1 < OCTAVE_DOG_IMAGES; ++layer) {
                ...
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        bool is_max = true;
                        bool is_min = true;
                        const float center = DoGs[1].at<float>(j, i);
                        ...
                        for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                            ...
                            const float neighbour = DoGs[1 + dz].at<float>(j + dy, i + dx);
                            is_min &= center < neighbour;
                        ...
                        {
                            const auto xShift = parabolaFitting(DoGs[1].at<float>(j, i - 1), center, DoGs[1].at<float>(j, i + 1));
...
bool phg::SIFT::buildDescriptor(const cv::Mat &img, float px, float py, double descrSampleRadius, float angle,
    ...
    for (int hstj = 0; hstj < DESCRIPTOR_SIZE; ++hstj) { // перебираем строку в решетке гистограмм
        for (int hsti = 0; hsti < DESCRIPTOR_SIZE; ++hsti) { // перебираем колонку в решетке гистограмм
            ...
            const float len2 = std::accumulate( sum, sum + DESCRIPTOR_NBINS, 0.0f, [](float a, float b) {
                return a + b * b;
            });
