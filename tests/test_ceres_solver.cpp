...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 25.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }

    // Давайте пофиксим знак
    if (line_params[0] * ideal_line[0] < 0) {
        for (int d = 0; d < 3; ++d) {
            line_params[d] *= -1;
        }
    }

    //  и найдем коэфициент прямой по которому наибольшее расхождение
    double diff = 0.0;
    int worst_d = 0;
    for (int d = 0; d < 3; ++d) {
        diff = std::max(diff, std::abs(line_params[d] - ideal_line[d]));
        worst_d = d;
    }
    double coef = ideal_line[worst_d] / line_params[worst_d];
    for (int d = 0; d < 3; ++d) {
        line_params[d] *= coef;
    }
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // менял задачу приведя ax + by + c = 0 -> ax + c = y, т.е. b == -1
        // residual[0] = (a*p.x + p.y + c) / ceres::sqrt(a*a + 1);
        // но расхождение становилось больше
    }
